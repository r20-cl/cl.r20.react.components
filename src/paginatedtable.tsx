  ...
  protected updatePage(): void {
    this.setState({
      ...
      (async () => {
        ...
        const paginationQuery = this.props.search.searchQuery ? {
    ...
    if (isHeader) {
      ...
      return (
        <td key={v4()}
          ...
          <input key={v4()} type="checkbox"
                 checked={this.state.selectedItemByPage[this.state.npage].find(it => it.id === item.id).selected}
